...
def build_emoji_dict(guild):
    """ Build an emoji dict for given guild, suitable as 'custom'
        argument to parse_emojis.
        Only emojis the bot is allowed to use are kept.
    """
...
def lookup_emoji(name, custom=None):
    try:
        return EMOJI_TABLE[name]
    except KeyError:
        pass
    try:
        return custom[name]
